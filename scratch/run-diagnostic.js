...
async function run() {
  ...
  try {
    ...
    const deployOut = execSync('clasp deploy -d "Diagnostic deployment"', { cwd: path.join(__dirname, '../google-apps-script') }).toString();
    console.log(deployOut);

    // Extract deployment ID
    // Deployment ID looks like: Deployed AKfycb... @XX
    ...
    console.log("4. Fetching headers diagnostics...");
    ...
    console.log("Diagnostic Data Result:\n", JSON.stringify(data, null, 2));

  } catch (err) {
    console.error("Execution error:", err);
  } finally {
    console.log("5. Restoring Client Script ID...");
